...
def read_rating_file(file_path):
...
def sample_100_with_tiers(df: pl.DataFrame,
        target_rows: int = 100) -> pl.DataFrame:
    tier_samples = []
...
def write_to_array_record(df_write: pl.DataFrame, out_file_path: str):
    ...
    try:
        for user_id, movie_id, rating, timestamp in df_write.select(
            ...
            record_tuple = (int(user_id), int(movie_id), int(rating),
                int(timestamp))
...
def verify_array_record(out_file_path: str):
    ...
    try:
        ...
        assert len(
            records) == 100, f"Expected 100 records in read_all, but found {len(records)}."
        ...
        for raw_bytes in records:
            ...
            assert isinstance(row,
                (list, tuple)), f"Expected a list or tuple, got {type(row)}."
...
res_dir = os.path.join(get_project_dir(), "src/test/resources/ml-1m/")

dfs = {}
for file_name in file_names_list:
    file_path = os.path.join(res_dir, f"{file_name}.dat")
    df = read_rating_file(file_path)
    dfs[file_name] = df

for fl1 in ["liked", "3", "disliked"]:
    name_train = f"ratings_train_{fl1}"
    name_val = f"ratings_val_{fl1}"
    name_test = f"ratings_test_{fl1}"
    
    df_train = dfs[name_train]
    df_val = dfs[name_val]
    df_test = dfs[name_test]
    
    common_users = (
        df_test.select("user_id")
        .unique()
        .join(df_train.select("user_id").unique(), on="user_id")
        .join(df_val.select("user_id").unique(), on="user_id")
    )

    test_filtered = df_test.join(common_users, on="user_id")
    train_filtered = df_train.join(common_users, on="user_id")
    val_filtered = df_val.join(common_users, on="user_id")
    ...
    # Apply the sampling to each split
