...
class RunInferenceImpl(beam.PTransform):
  ...
  def expand(self, examples: beam.PCollection) -> beam.PCollection:
    ...
    output_type = self.infer_output_type(examples.element_type) #<class 'tensorflow_serving.apis.prediction_log_pb2.PredictionLog'>
    ...
    #examples | "print_examples" >> beam.Map(lambda x: print(f"EXAMPLE: {x}"))
...
class _BaseSavedModelHandler(_BaseModelHandler):
  ...
  def _make_io_tensor_spec(self) -> _IOTensorSpec:
    ...
    io_tensor_specs = [] #[_IOTensorSpec(input_tensor_alias='examples', input_tensor_name='serving_default_examples:0', output_alias_tensor_names={'outputs': 'StatefulPartitionedCall_5:0'})]
  ...
  def _run_inference(self, examples: List[InputType],  # pytype: disable=signature-mismatch  # overriding-return-type-checks
    ...
    if hasattr(model, "run"):
      ...
      print(f'_run_inference saved_model branch')
      #assume is a saved_model, and
      signature = None
      try:
        signature = self._inference_spec_type.saved_model_spec.signature
      except AttributeError:
        signature = "serving_default"
      infer = model.signatures[signature]
      INPUT_KEY = list(infer.structured_input_signature[1].keys())[0]
      result = infer(**{INPUT_KEY: examples})
