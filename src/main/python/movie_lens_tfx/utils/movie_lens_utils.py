...
#========== tensor to tf_Example ==========
def convert_dict_inputs_to_tf_features(inputs_dict:Dict[str, Union[tf.Tensor, np.ndarray]]) -> List[tf.train.Features]:
  """
  given a dictionary of inputs of tensors or numpy arrays, return list of string serialized tf examples, one
  for each input batch element.
  Note this will fail for inputs_dict in graph mode.
  """
  features = []
  batch_size = inputs_dict['movie_id'].shape[0]
      ...
      for key, values in inputs_dict.items():
        ...
        if isinstance(value, tf.Tensor):
          ...
          if len(value) == 1:
            ...
            raise ValueError(f'ERROR: correct for:key={key}, values={values}, value={value}')
        if isinstance(value, bytes) or values.dtype in [str, tf.string]:
          f = tf.train.Feature(bytes_list=tf.train.BytesList(value=[value]))
        elif values.dtype in [float, tf.float32, tf.float64, tf.float16]:
          f = tf.train.Feature(float_list=tf.train.FloatList(value=[float(value)]))
        elif values.dtype in [int, bool, tf.int64, tf.int32, tf.int16]:
          f = tf.train.Feature(int64_list=tf.train.Int64List(value=[int(value)]))
        else:
          raise ValueError(f"evalue.dtype={value.dtype}, but only float, int, and str classes are handled.")
        feature_map[key] = f
    except Exception as ex:
      logging.error(f"ERROR: {ex}")
...
def convert_dict_inputs_to_tf_examples(inputs_dict:Dict[str, Union[tf.Tensor, np.ndarray]]) -> tf.train.Example:
  features_list = convert_dict_inputs_to_tf_features(inputs_dict)
  return [tf.train.Example(features=features) for features in features_list]
  
def convert_dict_inputs_to_tfexample_ser(inputs_dict:Dict[str, Union[tf.Tensor, np.ndarray]]):
  tf_examples_list = convert_dict_inputs_to_tf_examples(inputs_dict)
  return [tf_examples.SerializeToString() for tf_examples in tf_examples_list]
